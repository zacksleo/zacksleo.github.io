...
![API 认证与会话管理流程](https://blog.shaohushuo.com/images/鸿蒙智能体开发实战/image-32.1.png)
...
| 认证方式 | 作用域 | 验证方式 | 生命周期 |
|---------|-------|---------|---------|
| **平台级认证** | 执行引擎 → 服务端 | Access Key + HMAC 签名 | 长期有效，手动撤销 |
| **用户级授权** | 用户 → 服务端 | authCode → agentLoginSessionId | 单次会话，用户可解授权 |
...
| 状态 | 说明 | 触发时机 |
|------|------|---------|
| **initialized** | 会话已创建，等待激活 | `initialize` 方法调用 |
| **active** | 已激活，可正常通信 | `notifications/initialized` 调用 |
| **expired** | 已过期，需重新创建 | 超过 TTL（7天） |
| **revoked** | 已强制撤销 | 管理员撤销或安全策略触发 |
...
- [A2A 协议 - 会话管理](https://developer.huawei.com/consumer/cn/doc/service/agent2agent-comments-0000002500412353)
